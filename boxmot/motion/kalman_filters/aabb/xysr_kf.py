...
class KalmanFilterXYSR(object):
    ...
    def unfreeze(self):
        if self.attr_saved is not None:
            ...
            # x1, y1, s1, r1 = box1
            x1, y1, s1, r1 = box1[:4]# 修改
            w1, h1 = np.sqrt(s1 * r1), np.sqrt(s1 / r1)
            # x2, y2, s2, r2 = box2
            x2, y2, s2, r2 = box2[:4]# 修改
    ...
    def update(self, z, R=None, H=None):
        ...
        if H is None:
            ...
            if len(z) < self.dim_z:
                # 如果传入的测量值 z 比期望的维度要小
                # (这很可能是在 unfreeze 中创建的插值框，它缺少置信度等信息)
                # 我们用默认值(例如1.0)将其补齐到期望的维度
                padded_z = np.ones((self.dim_z, 1))
                padded_z[:len(z)] = np.array(z).reshape(-1, 1)
                z = padded_z
            # --- END: 插入结束 ---

            z = reshape_z(z[:self.dim_z], self.dim_z, self.x.ndim)
